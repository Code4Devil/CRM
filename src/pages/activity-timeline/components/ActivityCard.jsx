...
const ActivityCard = ({ activity, isLast }) => {
  ...
  const getActivityColor = (type) => {
    switch (type) {
      ...
      case 'deal_update': return 'text-orange-600 bg-orange-50';
            ...
            <div className="flex items-center space-x-3">
              {activity.contact?.avatar && (
                <Image
                  ...
                  alt={activity.contact.name || 'Contact Avatar'}
                  className="w-8 h-8 rounded-full object-cover"
                />
              )}
              <div>
                <div className="font-medium text-text-primary">{activity.contact?.name}</div>
                <div className="text-sm text-text-secondary">{activity.contact?.company}</div>
              </div>
            </div>

            <div className="flex items-center space-x-3 ml-auto">
              {activity.user !== 'System' && activity.user?.avatar && (
                <div className="flex items-center space-x-2">
                  <Image
                  ...
                  />
                  <span className="text-sm text-text-secondary">{activity.user?.name || activity.user}</span>
                </div>
              )}
            </div>
          </div>
